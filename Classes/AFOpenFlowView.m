...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if (newCover >= 0 && newCover < [array count]) {
		UILabel *title = CoverFlowTitle;
		
		NSString *string = [[NSString alloc] initWithString:[array objectAtIndex:newCover]];
		title.text = [NSString stringWithFormat:@"%@  %d of %d",
												[[string substringFromIndex:string.length - 17] substringToIndex:13]
												,newCover+1,[array count]];
		[string release];
	}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	CALayer *targetLayer = (CALayer *)[scrollView.layer hitTest:targetPoint];
	AFItemView *targetCover = [self findCoverOnscreen:targetLayer];
	selectedImageNum = targetCover.number;
